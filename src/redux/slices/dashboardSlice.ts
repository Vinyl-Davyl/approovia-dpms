...
const dashboardSlice = createSlice({
  ...
  reducers: {
    ...
    renameFolder: (state, action: PayloadAction<{ oldName: string; newName: string }>) => {
      const folder = state.folders.find((f) => f.name === action.payload.oldName);
      if (folder) folder.name = action.payload.newName;
    },
    renameProject: (state, action: PayloadAction<{ folderName: string; projectId: string; newName: string }>) => {
